...
import Animated, { withTiming, withSpring, useSharedValue, useAnimatedStyle, cancelAnimation, LinearTransition, runOnJS } from 'react-native-reanimated';
...
type TToastMessage = { item: { type?: 'success' | 'error' | 'notice', title?: string, message?: string }, onRemove: Function };

export const ToastMessage: React.FC<TToastMessage> = memo(({ item, onRemove }) => {
  const DURATION = 5000;
  ...
  const tapGesture = Gesture.Pan()
    ...
    .onFinalize(event => {
      if (event.translationX < limitPanSize) runOnJS(_onStartTimeout)();
      if (event.translationX >= limitPanSize) runOnJS(_onPanRemove)();
    });
