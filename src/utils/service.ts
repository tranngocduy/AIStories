...
import { KEYS_STORAGE, CHAPTER_FONT_SIZE, CHAPTER_LINE_HEIGHT, CHAPTER_COLORS } from '@/constants';
...
export const updateChapterSettings = (option: TChapSettings) => {
  ...
  setDataStorage({ key: KEYS_STORAGE.STORE_CHAPTER_SETTINGS, data: { chapterSettings } });
}

export const loadChapterSettings = () => {
  const initialData = getDataStorage({ key: KEYS_STORAGE.STORE_CHAPTER_SETTINGS });
