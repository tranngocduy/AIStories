...
export const PageVotes: React.FC<TPageFilterProps> = ({ query, onChangeFilter }) => {

  const items = [...FILTER_OPTION_VOTES];

  const [selected, setSelected] = useState<TOptionFilter>(query || items[0]);

  const _onSelect = (value: TOptionFilter) => setSelected(value);

  const _onPressApply = () => onChangeFilter({ type: 'votes', ...selected });
