...
import { FILTER_OPTION_RATING } from '@/constants';
...
export const PageRating: React.FC<TPageFilterProps> = ({ query, onChangeFilter }) => {

  const items = [...FILTER_OPTION_RATING];

  const [selected, setSelected] = useState<TOptionFilter>(query || items[0]);

  const _onSelect = (value: TOptionFilter) => setSelected(value);

  const _onPressApply = () => onChangeFilter({ type: 'rating', ...selected });
